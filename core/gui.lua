...
function SRT:GetMPlusData(flag)
    ...
    for idx, charTable in ipairs(sorted) do
        ...
        local r, g, b = self:GetClassColor(charTable.data.class)
        ...
        for k, v in ipairs(self.dungeonData) do
...
local function DrawMPlusGroup(container, flag)    
    ...
    for k, v in ipairs(SRT.dungeonData) do
...
function SRT:GetRenownData()
    ...
    for idx, charTable in ipairs(sorted) do
        ...
        charFrame:AddChild(charHeading)
        for i = 1, 4 do
            local cr, cg, cb = self:GetCovenantColorCode(i)
